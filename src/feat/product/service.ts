...
export class ProductService {
    ...
    static async updateProduct(id: string, user: { id: string; role: Role }, data: any) {
        ...
        return await prisma.$transaction(async (tx) => {
            // 0. Fetch previous size stock levels before they are deleted
            const prevSizes = await tx.productSize.findMany({
                where: { variant: { productId: realProductId } },
            ...
            // 2. Detect which sizes came back in stock (were previously out of stock/unavailable, now have stock > 0)
            const newSizes = await tx.productSize.findMany({
                where: { variant: { productId: realProductId } },
                include: { variant: true }
            });

            const backInStock: string[] = [];
            for (const newS of newSizes) {
                if (newS.stock > 0) {
                    const wasOutOfStock = prevSizes.length === 0 || prevSizes.some(prev =>
                        prev.size === newS.size &&
                        prev.variant.colorName === newS.variant.colorName &&
                        prev.stock === 0
                    );
                    if (wasOutOfStock) {
                        backInStock.push(`${newS.variant.colorName} - ${newS.size}`);
                    }
                }
            }
                ...
                if (userIds.length > 0) {
                    ...
                    for (const userId of userIds) {
                        await NotificationService.createNotification({
                            ...
                            message: `Good news! "${updatedProduct.title}" (Color/Size: ${backInStock.join(', ')}) is back in stock. Buy it now!`,
                            type: 'info',
