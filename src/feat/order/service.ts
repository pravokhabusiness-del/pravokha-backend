import { OrderStatus, PaymentStatus, Role } from '../../shared/domain/types';;
...
export class OrderService {
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            if (data.couponCode) {
                try {
                    ...
                    const initialSubTotal = products.reduce((sum, p) => {
                        const item = items.find(i => i.productId === p.id);
                        return sum + p.price * (item?.quantity || 1);
                    }, 0);
                    const subtotalAfterCombo = initialSubTotal - totalDiscount;

                    const validatedCoupon = await CouponService.validateCoupon(data.couponCode, subtotalAfterCombo);
                    couponIdToIncrement = validatedCoupon.id;

                    if (validatedCoupon.type === 'percent') {
                        couponDiscountAmount = Math.round(subtotalAfterCombo * validatedCoupon.value / 100);
                    } else {
                        couponDiscountAmount = validatedCoupon.value;
                    }
                    couponDiscountAmount = Math.min(couponDiscountAmount, subtotalAfterCombo);
                } catch (couponErr: any) {
                    throw new Error(`Coupon error: ${couponErr.message}`);
                }
            }
            ...
            let remainingDiscount = totalDiscount + couponDiscountAmount;
  ...
  remoteSurcharge: shippingResult.breakdown?.[0]?.remoteSurcharge
            ...
            if (couponIdToIncrement) {
                const { CouponService } = require('../coupon/service');
                await CouponService.incrementUsedCount(couponIdToIncrement, tx);
            }
