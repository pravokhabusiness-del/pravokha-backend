...
async function main() {
    ...
    const firstSeller = await prisma.vendor.findFirst();
    if (firstSeller && categories.length > 0) {
        for (const cat of categories) {
            for (let i = 1; i <= 2; i++) {
                const title = `${cat.name} Item ${i}`;
                const slug = `${cat.slug}-item-${i}`;
                await prisma.product.upsert({
                    ...
                    create: {
                        title,
                        slug,
                        description: `This is a premium ${cat.name} product.`,
                        price: Math.floor(Math.random() * 1000) + 500,
                        stock: 50,
                        categoryId: cat.id,
                        vendorId: firstSeller.id,
                        status: 'ACTIVE',
                        isVerified: true,
                        variants: {
                            create: {
